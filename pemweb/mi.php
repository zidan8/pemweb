<?php
$db_host = 'localhost'; // Nama Server
$db_user = 'root'; // User Server
$db_pass = ''; // Password Server
$db_name = 'sekolah'; // Nama Database

require "excel_reader.php";
//jika tombol import ditekan
if(isset($_POST['submit'])){
 
    $target = basename($_FILES['datami']['name']) ;
    move_uploaded_file($_FILES['datami']['tmp_name'], $target);
 
// tambahkan baris berikut untuk mencegah error is not readable
    chmod($_FILES['datami']['name'],0777);
    
    $data = new Spreadsheet_Excel_Reader($_FILES['datami']['name'],false);
    
//    menghitung jumlah baris file xls
    $baris = $data->rowcount($sheet_index=0);
    
//    jika kosongkan data dicentang jalankan kode berikut
    $drop = isset( $_POST["drop"] ) ? $_POST["drop"] : 0 ;
    if($drop == 1){
//             kosongkan tabel pegawai
             $truncate ="TRUNCATE TABLE mi";
             mysql_query($truncate);
    };
    
//    import data excel mulai baris ke-2 (karena tabel xls ada header pada baris 1)
    for ($i=2; $i<=$baris; $i++)
    {
//       membaca data (kolom ke-1 sd terakhir)
      $no     = $data->val($i, 1);
      $nama   = $data->val($i, 2);
      $alamat = $data->val($i, 3);
      $desa     = $data->val($i, 4);
      $kecamatan   = $data->val($i, 5);
      $kabupaten = $data->val($i, 6);
      $jenjang     = $data->val($i, 7);
      $status   = $data->val($i, 8);
      $nohp = $data->val($i, 9);
 
//      setelah data dibaca, masukkan ke tabel pegawai sql
      $query = "INSERT into pegawai (no,nama,alamat,desa,kecamatan,kabupaten,jenjang,status,nohp)values('$no','$nama','$alamat','$desa','$kecamatan','$kabupaten','$jenjang','$status','$nohp')";
      $hasil = mysql_query($query);
    }
    
    if(!$hasil){
//          jika import gagal
          die(mysql_error());
      }else{
//          jika impor berhasil
          echo "Data berhasil diimpor.";
    }
}


$conn = mysqli_connect($db_host, $db_user, $db_pass, $db_name);
if (!$conn) {
	die ('Gagal terhubung dengan MySQL: ' . mysqli_connect_error());	
}

$sql = 'SELECT * 
		FROM mi';
		
$query = mysqli_query($conn, $sql);

if (!$query) {
	die ('SQL Error: ' . mysqli_error($conn));
}
?>
<html>
	<head>
		<title>Database Siswa</title>
		<meta charset="utf-8" />
		<meta name="viewport" content="width=device-width, initial-scale=1" />
		<!--[if lte IE 8]><script src="assets/js/ie/html5shiv.js"></script><![endif]-->
		<link rel="stylesheet" href="assets/css/main.css" />
		<!--[if lte IE 8]><link rel="stylesheet" href="assets/css/ie8.css" /><![endif]-->
	</head>
	<body id="top">

		<!-- Header -->
			<header id="header">
				<div class="inner">
					<a href="#" class="image avatar"><img src="images/avatar.jpg" alt="" /></a>
					<h3><a href="index.php"> Home</a></h3>
					<h5><a href="mi.php"> Database sekolah MI</a></h5>
					<h5><a href="mts.php"> Database sekolah MTs</a></h5>
					<h5><a href="ma.php"> Database sekolah MA</a></h5>
					<h5><a href="sd.php"> Database sekolah SD</a></h5>
					<h5><a href="smp.php"> Database sekolah SMP</a></h5>
					<h5><a href="sma.php"> Database sekolah SMA</a></h5>
					<h5><a href="smk.php"> Database sekolah SMK</a></h5>
				</div>
			</header>

		<!-- Main -->
			<div id="main">

				<!-- One -->
					<section id="one">
						<table border="1" class="data-tabel">
						<h2>Database Sekolah MI</h2>
						<form name="myForm" id="myForm" onSubmit="return validateForm()" action="mi.php" method="post" enctype="multipart/form-data">
    						 <a href="import.xls" class="btn btn-default">Download Format   </a>
    						<input type="file" id="import" name="import" />
    						<span class="download"></span>
    						<input type="submit" name="submit" value="Import" /><br><br/>
						</form>
						<div class="row"></div>
							<thead>
							<tr>
								<th>no</th>
								<th>NPSN</th>
								<th>Nama</th>
								<th>Alamat</th>
								<th>Desa</th>
								<th>Kecamatan</th>
								<th>Kabupaten</th>
								<th>Jenjang</th>
								<th>Status</th>
								<th>Akreditasi</th>
								<th>No hp</th>
								<th>Jumlah Siswa</th>
							</tr>
							</thead>
							<tbody>
								<?php
								$total = 0;
								while ($row = mysqli_fetch_array($query))
								{
									$siswa  = $row['siswa'] == 0 ? '' : number_format($row['siswa'], 0, ',', '.');
									echo '<tr>
												<td>'.$row['no'].'</td>
												<td>'.$row['npsn'].'</td>
												<td>'.$row['nama'].'</td>
												<td>'.$row['alamat'].'</td>
												<td>'.$row['desa'].'</td>
												<td>'.$row['kecamatan'].'</td>
												<td>'.$row['kabupaten'].'</td>
												<td>'.$row['jenjang'].'</td>
												<td>'.$row['status'].'</td>
												<td>'.$row['akreditasi'].'</td>
												<td>'.$row['nohp'].'</td>
												<td>'.$siswa.'</td>
												</tr>';
												$total += $row['siswa'];
								}?>
							</tbody>
							<tfoot>
								<tr>
									<th colspan="11">TOTAL</th>
									<th><?=number_format($total, 0, ',', '.')?></th>
								</tr>
							</tfoot>
						</table>
						</section>
			</div>

		<!-- Footer -->
			<footer id="footer">
				
			</footer>

		<!-- Scripts -->
			<script src="assets/js/jquery.min.js"></script>
			<script src="assets/js/jquery.poptrox.min.js"></script>
			<script src="assets/js/skel.min.js"></script>
			<script src="assets/js/util.js"></script>
			<!--[if lte IE 8]><script src="assets/js/ie/respond.min.js"></script><![endif]-->
			<script src="assets/js/main.js"></script>

			<script type="text/javascript">
//    validasi form (hanya file .xls yang diijinkan)
    function validateForm()
    {
        function hasExtension(inputID, exts) {
            var fileName = document.getElementById(inputID).value;
            return (new RegExp('(' + exts.join('|').replace(/\./g, '\\.') + ')$')).test(fileName);
        }
 
        if(!hasExtension('filepegawaiall', ['.xls'])){
            alert("Hanya file XLS (Excel 2003) yang diijinkan.");
            return false;
        }
    }
</script>

	</body>

</html>